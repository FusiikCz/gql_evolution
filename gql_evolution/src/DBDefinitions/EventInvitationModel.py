...
class EventInvitationModel(BaseModel):
    ...
    # state_id references invitation states (invited, accepted, declined, etc.)
    state_id: Mapped[Optional[IDType]] = UUIDFKey(
        ForeignKey("event_invitation_states.id"),
        default=None,
        nullable=True,
        comment="State assigned to the invitation (invited, accepted, declined, etc.)"
    )
    ...
    state = relationship(
        ...
        primaryjoin="EventInvitationModel.state_id==EventInvitationStateModel.id",
        uselist=False,
        viewonly=True
    )
