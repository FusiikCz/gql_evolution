...
async def index():
    realpath = os.path.realpath("./src/Htmls/index.html")
    return realpath
...
async def endpoint_config_ui():
    realpath = os.path.realpath("./src/Htmls/endpoint_config.html")
    return realpath

@app.get("/test-report", response_class=FileResponse)
async def test_report_ui():
    realpath = os.path.realpath("./src/Htmls/test_report.html")
    return FileResponse(realpath, headers={"Cache-Control": "no-store"})
